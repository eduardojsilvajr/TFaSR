...
class RiverLoss(nn.Module):
    def __init__(self, unet: nn.Module, pos_thresh: float = 0.5, eps: float = 1e-7):
        super().__init__()
        self.unet = unet
    ...
    def forward(self, input, targets, base_max_list):
        ...
        loss = self.bce_logits(fake_river_heatmap, targets)
        ...
        for j in range(batchSize):
            real_j = np.reshape(targets[j].detach().cpu(), (H, W)).numpy()  # type: ignore
...
unet.to(device)
river_conterion = RiverLoss(unet=unet)
